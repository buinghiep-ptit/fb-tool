...
import { getCategoriesSort } from 'app/apis/shop/shop.service'
...
import { getPositions } from 'app/apis/players/players.service'
...
export default function PlayerDetail(props: Props) {
  ...
  const schema = yup
    .object({
      ...
      clothersNumber: yup.number().typeError('Nhâp số'),
      height: yup.number().typeError('Nhâp số'),
      weight: yup.number().typeError('Nhập số'),
      sizeShoes: yup.number().typeError('Nhâp số'),
      sizeSpikeShoes: yup.number().typeError('Nhâp số'),
      sizeClothers: yup.string(),
      subPosition: yup.string(),
      countMatch: yup.string(),
      cleanMatch: yup.string(),
      goal: yup.string(),
      yellowCard: yup.string(),
      redCard: yup.string(),
      oldClub: yup.string().required('Giá trị bắt buộc'),
      editor_content: yup.string().required('Giá trị bắt buộc'),
      prioritize: yup.string(),
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <FormControl style={{ width: '200px' }}>
                        ...
                        {!!methods.formState.errors?.subPosition?.message && (
                          <FormHelperText>
                            {methods.formState.errors?.subPosition.message}
                          </FormHelperText>
                        )}
